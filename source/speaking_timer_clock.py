...
I2C_NUM_ROWS = 2
I2C_NUM_COLS = 16
player = DFPlayer(0, 16, 17, 18) # Initialisierung DFPlayer (UART, TX-Pin, RX-Pin, Busy-Pin)
...
playlist = list(range(1, 46))  # Assuming you have 45 music files named 1.mp3, 2.mp3, ..., 45.mp3
...
sp_month = {1:'Jan', 2:'Feb', 3:'Mär', 4:'Apr', 5:'Mai', 6:'Jun', 7:'Jul', 8:'Aug', 9:'Sep', 10:'Okt', 11:'Nov', 12:'Dez'}
sp_dayname = {1:'Montag', 2:'Dienstag', 3: 'Mittwoch', 4:'Donnerstag', 5:'Freitag', 6:'Samstag', 7:'Sonntag'} 
...
#
def speak_frase(x): # 1- время, 2 - день недели, 3 - месяц и число, 4 - год, 1 впереди - на втором языке 
    if volume_on:
        if x == 1:
...
#
            ...
            player.playTrack(6, hour_dict.get(3)) #из папки 01 проговариваем день недели
            time.sleep(1)
# на немецком
        elif x == 12:
            player.playTrack(16, hour_dict.get(3)) #из папки 01 проговариваем день недели
            time.sleep(1)
#
        elif x == 3:
...
# на немецком
            ...
            player.playTrack(5, hour_dict.get(0)-2020) #из папки 05 проговариваем год
            time.sleep(1)
# на немецком
        elif x == 14:
            player.playTrack(15, hour_dict.get(0)-2020) #из папки 05 проговариваем год
            time.sleep(1)
...
def vol_onoff():
    print('button енкодера 2: Громкость on/off') # текущая громкость меняется на 0, предыдущее значение сохраняется
    global volume_on
    if volume_on:
        volume_on = False
...
def is_dfplayer_busy():
    return not DFPlayer_busy.value()  # Pin is low when DFPlayer is busy, so we invert the value
# 
def play_track():
    ...
    if volume_on:
        ...
        if not is_dfplayer_busy() and play_on and not playing:
            ...
            if current_track_index >= len(playlist):
                fisher_yates_shuffle(playlist) # Reset the playlist when all tracks have been played
...
# 2 button - проговаривание текущего времени, даты и года
def set_btn02():
    if volume_on:
        speak_frase(1)
        speak_frase(2)
...
# 2 button проговаривание текущего времени
btn02 = Button(21)
...
# выводим время
        print(f'{curr_hour:.2}' + ':'+ f'{curr_min:.2}' + ' Next Timer: ' + "%02d" % (num_alarm_spisok.get(num_alarm)))
        print(f'{curr_day:.2}' + '-' + sp_month.get(current_month) + '-' + f'{curr_year:02}' +' '+ sp_dayname.get(current_dayname))
        print(num_alarm)
        num_day_len_spisok = {1:10, 2:8, 3:8, 4:6, 5:9, 6:9, 7:9} # позиции для вывода дня недели 
        lcd.clear()
        lcd.move_to(0, 0)
        lcd.putstr("%02d:%02d" % (current_hour, current_minute))
        lcd.move_to(num_day_len_spisok.get(current_dayname), 0) # позиция вывода дня недели
        lcd.putstr(sp_dayname.get(current_dayname))
        lcd.move_to(0, 1)
        lcd.putstr(f'{curr_day:.2}' + '-' + sp_month.get(current_month) + '-' + f'{curr_year:4}' + ' ' + "%02d" % (minutes))
#
...
#говорим время каждый час         
              if (current_hour >= work_time_start and current_hour <=work_time_end-1) and current_minute == 0 and not time_spoken:
...
#                time.sleep(1)
              ...
              if (current_hour >= work_time_start and current_hour <=work_time_end-1) and current_minute == 30 and not time_spoken:
...
# будильник совпал
                time.sleep(0.6)
                print('Будильник ' + str(num_alarm) + 'совпал') # выводим сообщение будильника и устанавливаем флаг срабатывания будильника
                alarm_trigger = True
...
# на немецком
        ...
        if current_hour == current_timer_hour and current_minute == total_minutes and timer_running:
# время таймера
